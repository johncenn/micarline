...
class goods_class
{
	...
	public function update($id,$postData)
	{
        ...
        if(!$this->seller_id){
            ...
            $goodsUpdateData['plate'] = IFilter::act(IReq::get('plate'));
            $isjifen = IFilter::act(IReq::get('isjifen'),'int');    //是否是积分
            $duihuanpoint = IFilter::act(IReq::get('duihuanpoint'),'int');//兑换积分
            $good_type = IFilter::act(IReq::get('good_type'),'int');  //物流或到店
            
            $goodsUpdateData['good_type'] = $good_type;
            $goodsUpdateData['duihuanpoint'] = $duihuanpoint;
            $goodsUpdateData['isjifen'] = $isjifen;
            
            if($good_type == 3){
                
            }
            
            $seller_tichen = IFilter::act(IReq::get('tichen'));
            $sell_price = $goodsUpdateData['sell_price'];
            if($seller_tichen < 3.5 && $goodsUpdateData['xiaoshou_type']==0){
                //die("提成不符合要求，请提高点");
            }
            
            //销售提成比例
            $xiaoshou = ($seller_tichen-3.5) * 0.7;
            //三级提成比例
            $sanji = ($seller_tichen-3.5) * 0.3 / 3;
            //成本比例
            $chenben = 100 - $seller_tichen;
            
            $goodsUpdateData['sanjiticheng'] = $sanji ;
            $goodsUpdateData['chengben']    = $chenben;
            $goodsUpdateData['xiaoshouticheng'] = $xiaoshou;
            $goodsUpdateData['guishu'] = 0.5;
            $goodsUpdateData['shouxufei'] = 3;
            
            if($goodsUpdateData['xiaoshou_type'] == 1){
                $goodsUpdateData['sanjiticheng'] = 0;
                $goodsUpdateData['xiaoshouticheng'] = 0;
                $goodsUpdateData['guishu'] = 0;
                $goodsUpdateData['tichen'] = 0;
                $chenben = 100 -$goodsUpdateData['shouxufei'] ;
                $goodsUpdateData['chengben']    = $chenben;
           ...
           //print_r($goodsUpdateData);die;
            
            
            
            
            //$goodsUpdateData['good_type'  ]
            //$goodsUpdateData['duihuanpoint']
            
        }else{
            //商户上传
            $goodsUpdateData['plate'] = $sellerRow['plate_id'];
          ...
          //  echo "<br/>";
            ...
            //echo '归属是:'.$goodsUpdateData['guishu'];
            //die($goodsUpdateData['xiaoshou_type']);
            //销售提成比例
            $xiaoshou = ($seller_tichen-3.5) * 0.7;
            //三级提成比例
            $sanji = ($seller_tichen-3.5) * 0.3 / 3;
           ...
           // echo "销售提成比例是：".$xiaoshou." 三级提成/人：".$sanji."<br/>";
            //echo "成本是：".$chenben;
          // die; 
            $goodsUpdateData['sanjiticheng'] = $sanji ;
            $goodsUpdateData['chengben']    = $chenben;
       ...
       // $goodsUpdateData['plate'] = IFilter::act(IReq::get('plate'));
		//$goodsUpdateData['plate'] = $sellerRow['plate_id'];
		//echo "plate is ".$sellerRow['plate_id'];
       // echo "plate is ".$goodsUpdateData['plate'];
      // echo "isjifen is ".$isjifen;echo "<br/>";
       // echo "good_type is ".$good_type;echo "<br/>";
         //echo "duihuanpoint is ".$duihuanpoint;
