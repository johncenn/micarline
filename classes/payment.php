	...
	if($type == 'supplier')
		{
			...
			foreach($orderIdArray as $key => $order_id)
			{
			   ...
			   //     $vr_orderRow = $orderObj->getObj('order_no = '.$order_id.' and status = 1');
			    //}
				if(empty($orderRow))
				{
			...
			$goodsArray = json_decode($orderGoodsList['0']['goods_array'],TRUE);
			...
			$payment['M_goodsName'] = $goodsArray['name'];
			//用户信息
			...
			$payment['P_BuyerId']   = $orderRow['user_id'];
		...
		else if($type == 'vr_order')
		{
		    $orderNoArray = $argument;
		    
		    
		    $M_Amount     = 0;
		    $M_OrderNO    = array();
		    
		    foreach($orderNoArray as $key => $order_no)
		    {
		        //获取订单信息
		        //$orderObj = new IModel('order');
		        //$orderRow = $orderObj->getObj('id = '.$order_id.' and status = 1');
		        // if(empty($orderRow)){
		            $vr_orderObj = new IModel('vr_order');
		             $vr_orderRow = $vr_orderObj->getObj('order_no = '.$order_no.' and status = 1');
		        //}
		        ...
		        $orderGoodsList = $orderGoodsDB->query('order_no = '.$order_no);
		        foreach($orderGoodsList as $key => $val)
		        {
		            if(!goods_class::checkStore($val['goods_nums'],$val['goods_id'],$val['product_id']))
		            {
		        ...
		        $M_Amount   += $vr_orderRow['order_amount'];
		        $M_OrderNO[] = $vr_orderRow['order_no'];
		    }
		    //$goods = $orderGoodsList['0']['goods_array'];
		    $goodsArray = json_decode($orderGoodsList['0']['goods_array'],TRUE);
		    
		    $payment['M_Remark']    = $vr_orderRow['postscript'];
		    $payment['M_OrderId']   = $vr_orderRow['id'];
		    $payment['M_OrderNO']   = $vr_orderRow['order_no'];
		    $payment['M_Amount']    = $M_Amount;
		    $payment['M_BatchOrderNO'] = join("_",$M_OrderNO);
		    $payment['M_goodsName'] = $goodsArray['name'];
		    
		    //用户信息
		    $payment['P_Mobile']    = $vr_orderRow['mobile'];
		    $payment['P_Name']      = $vr_orderRow['accept_name'];
		    $payment['P_PostCode']  = $vr_orderRow['postcode'];
		    $payment['P_Telephone'] = $vr_orderRow['telphone'];
		    $payment['P_Address']   = $vr_orderRow['address'];
		    $payment['P_BuyerId']   = $vr_orderRow['buyer_id'];
		    //订单批量结算缓存机制
		    Order_Class::setBatch($payment['M_OrderNO'],$M_OrderNO);
