...
class CountSum
{
	...
	public function goodsCount($buyInfo,$promo='',$active_id='')
	{
    	...
    	else
    	{
	    	...
	    	if(isset($buyInfo['goods']['id']) && $buyInfo['goods']['id'])
	    	{
	    		...
	    		$goodsList  = $goodsObj->query('go.id in ('.$goodsIdStr.')','go.name,go.cost_price,go.market_price,go.id as goods_id,go.img,go.sell_price,go.point,go.weight,go.store_nums,go.exp,go.goods_no,0 as product_id,go.seller_id');
     ...
     */
    public function countOrderFee($goodsResult,$province_id,$delivery_id,$payment_id,$is_invoice,$discount = 0,$promo = '',$active_id = '',$good_type)
    {
    	//根据商家进行商品分组
    	$sellerGoods = array();
    ...
    public static function getSellerGoodsFeeQuery($seller_id = '',$start_time = '',$end_time = '',$is_checkout = '')
    {
        ...
        $where .= " and 'DATEDIFF(create_time,".ITime::getDateTime().")' >= 7";
    	
    	$orderGoodsDB = new IQuery('order');
    	$orderGoodsDB->order = "id desc";
    	$orderGoodsDB->where = $where;
    	file_put_contents("./order1.txt", '<?php return '.var_export( $orderGoodsDB,true).';?> ');
     ...
     * @param IQuery 结果集对象
     */
    public static function getSellerVrGoodsFeeQuery($seller_id = '',$start_time = '',$end_time = '',$is_checkout = '')
    {
        ...
        $vrOrderGoodsDB = new IQuery('vr_order');
        $vrOrderGoodsDB->order = "id desc";
        $vrOrderGoodsDB->where = $where;
        return $vrOrderGoodsDB;
    }
    	...
    	if($orderList && is_array($orderList))
    	{
    		...
    		foreach($orderList as $key => $item)
    		{
    		   ...
    		   // die("id is ".$good_id);
    		    ...
    		    if($item['order_type'] == "r"){
    		        $ever['order_type'] = "物流单";
    		    }else if($item['order_type'] == "vr"){
    		        $ever['order_type'] = "到店单";
    		    }
    			$ever['no'] = $item['order_no'];
    			$ever['sum'] = $item['real_amount']; //总价
    			$ever['jiesuan'] = $item['chengben'];
    			$ever['tichen'] = $ever['sum'] - $ever['jiesuan'];   //提成价格
    			$ever['refund'] = 0; //退款
    			$ever['create_date'] = $item['create_time'];
    			$ever['pay_time'] = $item['pay_time'];
    			$ever['p_tichen'] = $item['tichen'];   //提成百分比
    			
    			$ever['refund'] = 0 ;
     			//是否存在退款
    			$refundList = $refundObj->getObj("order_no = ".$item['order_no'].' and pay_status = 2');
    			
    			if(!empty($refundList)){
    			    $ever['refund'] = $refundList['amount'];
    			    
    			    
    			    $Refund_balance = $ever['sum'] - $ever['refund'];//退款剩余
    			    $good = new IQuery('goods as g');
    			    $where = ('g.id='.$good_id.'');
    			    $good->where = $where ;
    			    $result = $good->find();
    			    
    			    $p_shouxufei = $result[0]['shouxufei'];//手续费的百分比
    			    $shouxufei = $ever['sum'] * $p_shouxufei; //手续费用 (含有百分比)
    			    
    			    $p_over = $ever['p_tichen'] - $p_shouxufei ; //剩下的费用百分比(归属、三级提成、销售提成)
    			    $over = $Refund_balance * $p_over;   //剩下的费用
    			    
    			    
    			    $finally_jisuan = $Refund_balance - ($shouxufei + $over) / 100 ;
    			    
    			    
    			}else{
    			    $finally_jisuan = $item['chengben'];
    			}
    			
    			
    			
    			/* echo "ok";
    			$finally_jisuan = ( ($ever['sum'] - $ever['refund'] ) * ( $ever['p_tichen']-3 ) / 100 );
    			echo $finally_jisuan;
    			//echo "ever['sum'] - ever['refund']".$ever['sum'] - $ever['refund'];
    			die; */
    			//最终结算                                                                           //26.4                      //
    			//$finally_jisuan = $ever['sum'] - $ever['refund'] - ( ($ever['sum']*3/100) + ( ($ever['sum'] - $ever['refund'] ) * ( $ever['p_tichen']-3 ) / 100 ) );
    			/* echo $finally_jisuan;
    			die; */
    			$ever['real_Sum'] = $finally_jisuan;
    			$true_jiesuan = $true_jiesuan + $finally_jisuan;
    			array_push($into,$ever);
    		}
    	}
