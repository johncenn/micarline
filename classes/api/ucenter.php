...
class APIUcenter
{
	...
	public function getMemberTongJi($userid){
		...
		$info = $query->find();
		$vr_query = new IQuery('vr_order');
		$vr_query->fields = "count(id) as num";
		$vr_query->where  = "buyer_id = ".$userid." and if_del = 0";
		$vr_info = $vr_query->find();
		$result['num'] = $info[0]['num']+$vr_info[0]['num'];
	...
	//用户中心-虚拟订单列表
	public function getVrOrderList($userid){
	    $page = IReq::get('page') ? IFilter::act(IReq::get('page'),'int') : 1;
	    $query = new IQuery('vr_order');
	    $query->where = "buyer_id =".$userid." and if_del= 0";
	    $query->order = "id desc";
	    $query->page  = $page;
	    return $query;
	}
	//用户中心-虚拟订单列表商品名
	public function getVrOrderGoodName($order_no){
	    $query = new IQuery('vr_order_goods');
