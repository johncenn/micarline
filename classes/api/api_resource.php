...
return array(
	...
	'getCommendRecom' => array(
		'query' => array(
			'name' => 'commend_goods as co',
			'join' => 'left join goods as go on co.goods_id = go.id',
			'where' => 'co.commend_id = 4 and go.is_del = 0 AND go.id is not null and go.isjifen != 1',
			'fields' => 'go.img,go.sell_price,go.name,go.id,go.market_price,go.plate',
			'limit'=>'10',
			'order'=>'sort asc'
		)
	),
	...
	//根据商品分类取得商品列表(如果是积分商品就不取出来)
	'getCategoryExtendList'=>array(
	    'query'=>array(
	    	'name'  => 'category_extend as ca',
	    	'join'  => 'left join goods as go on go.id = ca.goods_id',
	    	'where' => 'ca.category_id in(#categroy_id#) and go.is_del = 0 and go.isjifen != 1 and go.xiaoshou_type=0',
	    	'order' => 'go.sort asc',
	    	'fields'=> 'go.id,go.name,go.img,go.sell_price,go.market_price,go.xiaoshou_type',
	    	'limit' => 10,
	    )
	),
	//根据分类取销量排名列表
    ...
    //根据一级分类输出二级分类列表
    'getWholesaleGoods'=>array(
        'query'=>array(
            'name'  => 'goods',
            'where' => ' is_del != 1 and xiaoshou_type = 1 ',
            'limit' => 1,
        )
    ),
	//所有品牌列表
    ...
    //用户中心-虚拟订单列表
    'getVrOrderList' => array(
        'file' => 'ucenter.php','class' => 'APIUcenter'
    ),
    //用户中心-虚拟订单列表商品名
    'getVrOrderGoodName' => array(
        'file' => 'ucenter.php','class' => 'APIUcenter'
    ),
	//用户中心-订单中商品列表
    ...
    //用户中心-订单中商品列表 -退款
    //用户中心-订单中商品列表
    'getVrOrderGoodsListByGoodsid2'=>array(
        'query'=>array(
            'name'  => 'vr_order_goods as og',
            'join'  => 'left join goods as go on og.goods_id = go.id',
            'where' => 'vr_order_no = #order_no# ',
            'fields'=> 'og.*,go.point',
        )
    ),
    
    //用户中心-订单中商品列表
    'getVrOrderGoodsListByGoodsid'=>array(
        'query'=>array(
            'name'  => 'vr_order_goods as og',
            'join'  => 'left join goods as go on og.goods_id = go.id',
    ...
    //积分购物的快递方式
    'getDeliveryListByjifen' => array(
        'query' => array(
            'name' => 'delivery',
            'where'=> 'is_delete = 0 and status = 1 and id = 1',
        )
    ),
