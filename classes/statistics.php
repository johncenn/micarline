...
class statistics
{
	 ...
	 * @return string
	 */
	public static function getPlateName($plates_json)
	{
	   
	    
	    $plates = json_decode($plates_json,true);
	    $res = "";
	    foreach($plates as $v){
	        $res = $res . $v . "," ;
	    }
	    $str_length = strlen($res);
	    $plates = substr($res,0,$str_length-1);
	     
	    
	    
	    $plateDB = new IQuery('plate');
	    $plateDB->fields = "plate_name";
	    $plateDB->where = "plate_id in($plates)";
	    $result = $plateDB->find();
	    foreach ($result as $v){
	        echo $v['plate_name']."　";
	    }
