...
class Order_Class
{
	 ...
	 * @param $order_id 订单ID
	 */
	public static function addVrGoodsCommentChange($order_id)
	{
	    //获取订单对象
	    $orderDB  = new IModel('order');
	    $orderRow = $orderDB->getObj('id = '.$order_id);
	 ...
	 * @return false or int order_id
	 */
	public static function updateSupplierOrderStatus($orderNo,$admin_id = '',$note = '')
	{
	    //获取订单信息
	    ...
	    else if($orderRow['pay_status'] == 0)
	    {
	        ...
	        {
	            //减少库存量
	            $orderGoodsDB = new IModel('supplier_order_goods');
	            $orderGoodsList = $orderGoodsDB->query('order_id = '.$orderRow['id']);
	            $orderGoodsListId = array();
	            foreach($orderGoodsList as $key => $val)
	            {
	 ...
	 * @return false or int order_id
	 */
	public static function updateVrOrderStatus($orderNo,$admin_id = '',$note = '')
	{
	    //获取订单信息
	    $orderObj  = new IModel('vr_order');
	    $orderRow  = $orderObj->getObj('order_no = "'.$orderNo.'"');
	    ...
	    else if($orderRow['pay_status'] == 0)
	    {
	        ...
	        {
	            //减少库存量
	            $orderGoodsDB = new IModel('vr_order_goods');
	            $orderGoodsList = $orderGoodsDB->query('order_no = '.$orderRow['order_no']);
	            $orderGoodsListId = array();
	            foreach($orderGoodsList as $key => $val)
	            {
	                $orderGoodsListId[] = $val['id'];
	            }
	            self::updateVrStore($orderRow['order_no'],'reduce');
	        }
	 ...
	 * @param string $type 增加或者减少 add 或者 reduce
	 */
	public static function updateVrStore($orderGoodsNO,$type = 'add')
	{
	    if(!is_array($orderGoodsNO))
	    {
	        $orderGoodsNO = array($orderGoodsNO);
	    }
	
	    $newStoreNums  = 0;
	    $updateGoodsId = array();
	    $orderGoodsObj = new IModel('vr_order_goods');
	    $goodsObj      = new IModel('goods');
	    $productObj    = new IModel('products');
	    $goodsList     = $orderGoodsObj->query('id in('.join(",",$orderGoodsNO).') and is_send = 0','goods_id,product_id,goods_nums,seller_id');
	 ...
	 * @return array()
	 */
	public function getSupplierOrderShow($order_id,$user_id = 0,$seller_id = 0)
	{
	    $where = 'id = '.$order_id;
	    if($user_id !== 0)
	    {
	    ...
	    if($data)
	    {
	        ...
	        //获取商品总重量和总金额
	        $tb_order_goods = new IModel('supplier_order_goods');
	        $order_goods_info = $tb_order_goods->query('order_id='.$order_id);
	        $data['goods_amount'] = 0;
	        $data['goods_weight'] = 0;
	 ...
	 * @return array()
	 */
	public function getVrOrderShow($order_no,$user_id = 0,$seller_id = 0)
	{
	    $where = 'order_no = '.$order_no;
	    if($user_id !== 0)
	    {
	        $where .= ' and buyer_id = '.$user_id;
	    }
	    ...
	    if($data)
	    {
	        ...
	        //获取商品总重量和总金额
	        $tb_order_goods = new IModel('vr_order_goods');
	        $order_goods_info = $tb_order_goods->query('order_no='.$order_no);
	        $data['goods_amount'] = 0;
	        $data['goods_weight'] = 0;
	        ...
	        $query->join = ' left join member as m on u.id=m.user_id ';
	        $query->fields = 'u.username,m.email,m.mobile,m.contact_addr,m.true_name';
	        $query->where = 'u.id='.$data['buyer_id'];
	...
	/**
	 * @brief  把积分商品同步到order_goods表中
	 */
	public function insertjifenOrderGoods($order_id,$goodsResult = array(),$dataArray){
	    $orderGoodsObj = new IModel('order_goods');
	    ...
	    if(isset($goodsResult['goodsList']))
	    {
	        foreach($goodsResult['goodsList'] as $key => $val)
	        {
	            ...
	            $goodsArray['img']         = $val['img'];
	            $goodsArray['goods_price'] = $val['sell_price'];
	            $goodsArray['real_price']  = $dataArray['payable_amount'];
	            $goodsArray['goods_nums']  = $val['count'];
	            $goodsArray['goods_weight']= $val['weight'];
	            $goodsArray['goods_array'] = IFilter::addSlash(JSON::encode($specArray));
	            $goodsArray['seller_id']   = $val['seller_id'];
	            $goodsArray['duihuanpoint'] = $dataArray['duihuanpoint'];
	            $goodsArray['isjifen']  = 1;
	            $orderGoodsObj->setData($goodsArray);
	            $orderGoodsObj->add();
	        }
	    }
	 ...
	 * @param $goodsInfo 商品和货品信息（购物车数据结构,countSum 最终生成的格式）
	 */
	public function insertSupplierOrderGoods($order_id,$goodsResult = array())
	{
	    $orderGoodsObj = new IModel('supplier_order_goods');
	 ...
	 * @param $goodsInfo 商品和货品信息（购物车数据结构,countSum 最终生成的格式）
	 */
	public function insertVrOrderGoods($order_no,$goodsResult = array())
	{
	    $orderGoodsObj = new IModel('vr_order_goods');
	
	    //清理旧的关联数据
	    $orderGoodsObj->del('order_no = '.$order_no);
	 ...
	 * 到店积分对应的方法
	 */
	public function insertVrjifenOrderGoods($order_no,$goodsResult = array(),$dataArray){
	    $orderGoodsObj = new IModel('vr_order_goods');
	    ...
	    if(isset($goodsResult['goodsList']))
	    {
	        foreach($goodsResult['goodsList'] as $key => $val)
	        {
	            ...
	            $goodsArray['img']         = $val['img'];
	            $goodsArray['goods_price'] = $val['sell_price'];
	            $goodsArray['real_price']  = $dataArray['order_amount'];
	            $goodsArray['goods_nums']  = 1;
	            $goodsArray['goods_weight']= $val['weight'];
	            $goodsArray['goods_array'] = IFilter::addSlash(JSON::encode($specArray));
	            $goodsArray['seller_id']   = $val['seller_id'];
	            $goodsArray['duihuanpoint'] = $dataArray['duihuanpoint'];
	 ...
	 * @return int 订单状态值 0:未知; 1:未付款等待发货(货到付款); 2:等待付款(线上支付); 3:已发货(已付款); 4:已付款等待发货; 5:已取消; 6:已完成(已付款,已收货); 7:全部退款; 8:部分发货(货到付款+已经付款); 9:部分退款(未发货+部分发货); 10:部分退款(全部发货); 11:已发货(货到付款); 12:未处理的退款申请
	 */
	public static function getVrOrderStatus($orderRow)
	{
	    //1,刚生成订单,未付款
	    if($orderRow['status'] == 1)
	    {
	...
	//获取虚拟订单支付状态
	public static function getVrOrderPayStatusText($orderRow)
	{
	    if($orderRow['status'] == '6')
	    {
	        return '全部退款';
	 ...
	 */
	public static function vrorderStatusText($statusCode)
	{
		$result = array(
			0 => '未知',
			1 => '等待服务',
	 ...
	 * @param string $sendor 操作者所属 admin,seller
	 */
	public static function sendSupplierGoods($order_id,$order_goods_relation,$sendor_id,$sendor = 'admin')
	{
	    $order_no = IFilter::act(IReq::get('order_no'));
	 ...
	 * @return array 查询条件（$join,$where）数据组
	 */
	public static function getVrSearchCondition($search)
	{
	    $join  = "left join payment as p on o.pay_type = p.id";
	    $where = "if_del = 0";
	    //条件筛选处理
	    if(isset($search['is_seller']))
	    {
	...
	public static function getBatch($orderKey)
	{
		...
		{ 
		    ...
		    $supplierDB = new IModel('supplier');
			foreach($orderList as $key => $val)
			{
				...
				if($orderRow)
				{
				    ...
				    $vr_orderRow = $vr_orderDB->getObj('order_no = "'.$val.'"');
				    if($vr_orderRow)
				    {
				        $result[$val] = $vr_orderRow['order_amount'];
				    }else{
				        $supplierRow = $supplierDB->getObj('order_no = "'.$val.'"');
				        if ($supplierRow){
				            $result[$val] = $supplierRow['order_amount'];
				        }
				    }
				}
			}
