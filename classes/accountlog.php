...
class AccountLog
{
	...
	public static function sellerBillTemplate($countData = null)
	{
	    ...
	    //$orderlist = $countData['tichen'];
	    //file_put_contents("./test.txt","tichen is ".$orderlist);
	    
		$replaceData = array(
			'{startTime}'        => $countData['start_time'],
			'{endTime}'          => $countData['end_time'],
			'{orderAmountPrice}' => $countData['orderAmountPrice'],  //订单结算总金额
			'{refundFee}'        => $countData['refundFee'],         //退款总金额
			'{countFee}'         => $countData['countFee'],          //55
			'{orgCountFee}'      => $countData['orgCountFee'],       //55
			'{orderNum}'         => $countData['orderNum'],
			'{platformFee}'      => $countData['platformFee'],       //0
			'{orderNoList}'      => join(",",$countData['orderNoList']),
			'{commissionPer}'    => $countData['commissionPer'],
			'{commission}'       => $countData['commission'],
		    '{mjiesuan}'         => $countData['mjiesuan'],
		    '{shouxufei}'        => $countData['shouxufei'],
		    '{tichen}'           => $countData['tichen'],
		    '{payable_amount}'   => $countData['payable_amount'],
		);
		...
		$templateString = "结算起止时间：【{startTime}】到【{endTime}】，订单号：【{orderNoList}】，订单数量共计：【{orderNum}单】，商家实际结算金额：【￥{mjiesuan}】，结算金额计算明细：【订单商品金额：￥{payable_amount}】-【退款总金额：￥{refundFee}】+【平台促销活动金额：￥{platformFee}】-【结算手续费：￥{shouxufei}】-【提成：￥{tichen}】";
