...
class wechat extends pluginBase
{
	...
	public static function setOpenId($openid)
	{
		...
		$weuser = $oauth_user->getObj("oauth_user_id = '$openid' ");
		$f_id = $weuser['user_id'];
	...
	public function bindUser($oauthAccess)
	{
		...
		else
		{
			...
			//查出邀请者的信息
			$yaoqingId = ISession::get('yaoqingId');
			if(!empty($yaoqingId)){
			    
    			$yaoMessages = $memberDB->getObj('user_id = "'.$yaoqingId.'"');
	...
	public function oauthUrl($url,$snsType = "snsapi_userinfo")
	{
	    ...
	    //授权登陆
	    //print_r($_GET['yaoqing']);
	    $yaoqingId = $_GET['yaoqing'];
	    ISession::set('yaoqingId',$yaoqingId);
