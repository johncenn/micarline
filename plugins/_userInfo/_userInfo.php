...
class _userInfo extends pluginBase
{
	...
	public function userRegAct()
	{
    	...
    	$yaoqingren = IFilter::act(IReq::get('yaoqingren','post'));//获取邀请人
...
//     	die;
		...
		//判断邀请人是否存在
		$userObj = new IModel('user');
		$yaoqingrenRow = $userObj->getObj('username = "'.$yaoqingren.'"');
		
		//判断是否有邀请人
		if($yaoqingrenRow){
		    //邀请人id
		    $yaoqingId = $yaoqingrenRow['id'];
		    $yaoObj = new IModel('member');
		    //邀请人信息
		    $yaoMessages = $yaoObj->getObj('user_id = "'.$yaoqingId.'"');
