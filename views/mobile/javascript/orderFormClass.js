 ...
 */
function orderFormClass(isjifen=0,price=0)
{
	_self = this;
	...
	this.doAccount = function()
	{
		...
		if(isjifen == 1){
			//最终金额
			this.orderAmount = parseFloat(price) - parseFloat(this.ticketPrice) + parseFloat(this.deliveryPrice) + parseFloat(this.paymentPrice) + parseFloat(this.taxPrice) + parseFloat(this.protectPrice);
			
		}else{
			console.log("inxxdyyy");
			//最终金额
		...
		console.log("finally is "+this.orderAmount);
		//刷新DOM数据
