 ...
 */
function productClass(goods_id,user_id,promo,active_id,isjifen,jifen)
{
	if(!isjifen){
		isjifen = 0;
	}
	
	//console.log("isjifen111 is" +isjifen);
	//console.log("jifen1111 is "+jifen);
	...
	this.delivery = function(provinceId,provinceName)
	{
		...
		$.getJSON(creatUrl("block/order_delivery"),{'province':provinceId,'goodsId':goodsId,'productId':productId,'isjifen':isjifen,'num':buyNums,'random':Math.random},function(json)
		{
			//清空配送信息
			$('#deliveInfo').empty();
			express_obj = json ;
			...
			//回显到快递价格
			$(".express_pirce").html(json[1].price);
			...
			if(_self.checkSpecSelected() == true)
			{
				...
				$.getJSON(creatUrl("site/getProduct"),{"goods_id":_self.goods_id,"specJSON":specJSON,"random":Math.random},function(json){
					if(json.flag == 'success')
					{
						...
						//设置数量最大库存值
						$(".store_num_s").attr("max",json.data.store_nums);
					}
					else
					{
						alert(json.message);
		...
		$.getJSON(creatUrl("simple/joinCart"),{"goods_id":goods_id,"type":type,"goods_num":buyNums,"random":Math.random},function(content)
		{
			if(content.isError == false)
			{
				...
				$.getJSON(creatUrl("simple/showCart"),{"random":Math.random},function(json)
				{
					...
					hui.iconToast(content.message,"success");
				});
			}
			else
			{
	...
	this.buyNow = function(jifen = 0)
	{
		...
		if(isjifen == 1){
			...
			var buyNums = 1;
			//普通购买
			var url = "/simple/cart2/id/"+id+"/num/"+buyNums+"/isjifen/"+1+"/jifen/"+jifen+"/type/"+type+"//";
			console.log("in 1");
		}else{
			//设置必要参数
			var buyNums = parseInt($.trim($('#buyNums').val()));
	...
	!(function()
	{
		...
		//立即购买按钮
		$('#buyNowButton').bind('click',function(){
			
			var userpoint = $("#buyNums2").val();
			if(!userpoint){
				userpoint = 0;
			}
			console.log("立即购买按钮 "+userpoint);
			_self.buyNow(userpoint);
		});
		
		//立即购买按钮2
		$('#buyNowButton2').bind('click',function(){
