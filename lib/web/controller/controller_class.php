...
class IController extends IControllerBase
{
	...
	public function getViewFile($viewName)
	{
	   ...
	   // $site_config   = $siteConfigObj->getInfo();
		...
		//echo $path;//ppppp
	   //$plateType = $_GET['bankuai'];
	   //echo "bankuai is ".IFilter::act(ISession::get('plate_type'));
	 /*  $session_plate = IFilter::act(ISession::get('plate_type'));
	   //如果带过来的板块参数为空
   	    if(empty($plateType)){
   	        //echo "b_k<br/>";
   	        //session为空 且 参数为空
   	        if(empty($session_plate)){
   	           // echo "s_k<br/>";
   	            $plateType = 1;
   	            ISession::set('plate_type',$plateType);
   	        }
   	    }else{
   	       // echo "b_nk";
   	       //参数不空  且 session 为空
   	       if(empty($session_plate)){
   	           ...
   	           //参数不空 且 session 不空, 且 不相等 ，保存参数的板块id
   	           if($plateType != $session_plate){
   	             //  echo "n q";
   	               ISession::set('plate_type',$plateType);
   	           }
   	       }
