...
class Order extends IController implements adminAuthorization
{
	...
	public function vr_order_show()
	{
	    ...
	    {
	        $order_show = new Order_Class();
	        $data = $order_show->getVrOrderShow($order_no);
	        $vr_order_codeObj = new IModel('vr_order_code');
            $this->vr_order_codeinfo = $vr_order_codeObj->query('order_no = '.$order_no);
	        if($data)
	        {
	            $this->setRenderData($data);
    ...
    public function vr_order_edit()
    {
        ...
        if($order_no)
        {
            ...
            $data    = $orderDB->getObj('order_no = '.$order_no);
            if(Order_class::getVrOrderStatus($data) >= 3)
            {
                IError::show(403,"当前订单状态不允许修改");
            }
            ...
            //获取订单中的商品信息
            $orderGoodsDB         = new IQuery('vr_order_goods as og');
            $orderGoodsDB->join   = "left join goods as go on og.goods_id = go.id left join products as p on p.id = og.product_id";
            $orderGoodsDB->fields = "go.id,go.name,p.spec_array,p.id as product_id,og.real_price,og.goods_nums,go.goods_no,p.products_no";
            $orderGoodsDB->where  = "og.order_no = ".$order_no;
            $this->orderGoods     = $orderGoodsDB->find();
            //获取用户名
            if($data['buyer_id'])
            {
                $userDB  = new IModel('user');
                $userRow = $userDB->getObj("id = ".$data['buyer_id']);
                $this->username = isset($userRow['username']) ? $userRow['username'] : '';
            }
        }
        $this->redirect('order_edit');
    ...
    public function vr_order_list()
    {
        ...
        $page   = IReq::get('page') ? IFilter::act(IReq::get('page'),'int') : 1;
        //条件筛选处理
        list($join,$where) = order_class::getVrSearchCondition($search);
        //拼接sql
	...
	function vr_order_exchange_show(){
	    ...
	    if(!empty($vr_code)){
	        if(strlen($vr_code) == 18){
	            ...
	            $vrOrderCodeList = $vrOrderCodeObj->getObj('vr_code = '.$vr_code);
