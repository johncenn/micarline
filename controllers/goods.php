...
class Goods extends IController implements adminAuthorization
{
	...
	function category_save()
	{
        ...
        //获取板块id
        $plate = IFilter::act(IReq::get('plate'));
	...
	function store_edit(){
	   // echo "get";
	    ...
	    $result = $good->query("id='$good_id'");
	    //echo "id is ".$good_id ;
	   //print_r($result);
	   
	    $this->setRenderData(array('good'=>$result));
	    $this->redirect('stroe_update',false);
	...
	function store_update(){
	   // print_r($_POST);
	    ...
	    $date = array(
	        "sanjiticheng"    => $_POST['sanjitichen'],
	        "chengben"        => $_POST['chenben'],
	        "guishu"          => $_POST['guishu'],
	        "xiaoshouticheng" => $_POST['xiaoshouticheng'],
	        "shouxufei"       => $_POST['shouxufei']
	    );
	    
	    $goodDB = new IModel("goods");
	    $goodDB->setData($date);
	    $res = $goodDB->update("id = $good_id");
	    if($res == 0){
