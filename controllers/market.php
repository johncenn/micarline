...
class Market extends IController implements adminAuthorization
{
	...
	public function bill_update()
	{
		...
		//echo "id is $id<br/>";
		//echo "pay content is $pay_content<br/>";
		...
		if($id)
		{
			...
			/*  echo "<pre>";
			//print_r($billRow['log']);
			$getdata = json_decode($billRow['log'],true);
			...
			if(isset($billRow['log']) && $obj)
			{
			    
				//更新订单商品关系表中的结算字段
				$orderDB = new IModel('order');
				$vrOrderDB = new IModel('vr_order');
				$member = new IModel('member');
				$seller_balance = 0;//商家结算金额
				
				//$orderIdArray = explode(',',$billRow['order_ids']);
				foreach($obj as $key => $val)//val is order id
				{
				   /*  echo "key is ";
				    print_r($key);
				    echo "<br/>";
				    ...
				    //查出当前order表的信息       $db->where    = "pay_status = 1";
				    $orderinfo = $orderDB->query("order_no='$no'");
				    $orders = $orderinfo;
				    
				    if(empty($orders)){
				        $orders = $vrOrderDB->query("order_no='$no'");
				    }
				    
				    /* echo "<pre>";
				    print_r($orders);
				    echo "</pre>"; */
				    //echo $orders[0]['chengben'];
				    
				   
				    //更新用户账户 s 和 商家账户
				    if($is_pay == 1){
				        $temp_balance = $orders[0]['chengben'];
				        
				        //商家结算额计算
				        $seller_balance = $seller_balance + $temp_balance;
				        
				       // echo $seller_balance;
				        //die;
    				    $one = $orders[0]['one'];
    				    $two = $orders[0]['two'];
    				    $three = $orders[0]['three'];
    				    $guishu = $orders[0]['guishuid'];
    				    $xiaoshou = $orders[0]['xiaoshouid'];
    				   
    				    //没有退款则更新会员的账户收入
                        if($val['refund'] == 0 ){
                            
                            //更新会员的三级提成
                            $member->setData(array('balance' => 'balance + '.$orders[0]['sanjiticheng']));
                            $member->update("user_id in ($one,$two,$three) ",'balance');
                            
                            //更新归属
                            if($guishu != 0){
                                $member->setData(array('balance' => 'balance + '.$orders[0]['guishu']));
                                $member->update("user_id = ".$guishu,'balance');
                            }
                             
                            //销售提成
                            if($xiaoshou != 0){
                                $member->setData(array('balance' => 'balance + '.$orders[0]['xiaoshouticheng']));
                                $member->update("user_id = ".$xiaoshou,'balance');
                            }
                        }
				    ...
				    if(!empty($orderinfo)){
    					...
    					$orderDB->update('order_no = '.$no);
				    }else{
				        $vrOrderDB->setData(array(
				            'is_checkout' => $is_pay,
				            'check_time'  =>ITime::getDateTime(),
				            	
				        ));
				        $vrOrderDB->update('order_no = '.$no);
				    }
				}
			...
			//更新账户
			$seller->setData(array('seller_balance' => 'seller_balance + '.$seller_balance));
			$seller->update("id=$seller_id",'seller_balance');
    ...
    public function orderxq(){
        ...
        $result = $orderDB->getObj($where);
        
        if(empty($result)){
            $result = $vrOrderDB->getObj($where);
        }
        
        $data = array(
            "res"=>$result,
            "jiesuan"=>$_GET['jiesuan'],
            "real_sum"=>$_GET['real_sum'],
            "refund"=>$_GET['refund']
        );
        $this->setRenderData($data);
        $this->redirect('orderxq');
    }
	...
	function detailsUpdate(){
	    ...
	    $orderDB->setData(array("final_settl" => 1));
	    $res = $orderDB->update("order_no= '$order_id' ");
	  // echo $res;
	  //  $this->seller_details();
