...
class Member extends IController implements adminAuthorization
{
	...
	public function seller_add()
	{
		...
		$seller_type    = IFilter::act(IReq::get('seller_type'),'int');
		...
		if(!$seller_id)
		{
			...
			$userDB = new IModel('user');
			$userData = $userDB->getObj('username = "'.$seller_name.'"');
			if (!$userData){
			    $userRow = array(
			    ...
			    $user_id = $userDB->add();
			    $sellerRow['member_id'] = $user_id;
			    $sellerRow['member_name'] = $seller_name;
			    if ($user_id){
			        $memberDB = new IModel('member');
			        $memberRow = array(
			            'user_id'=>$user_id,
