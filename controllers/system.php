...
class System extends IController implements adminAuthorization
{
    ...
    */
	...
	//添加板块,修改【单页】数据回显                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      
	function bankuai_edit(){
	    $id =IFilter::act( IReq::get('id'),'int' );
	    if($id){
	        $bankuaiObj = new IModel('plate');
	       
	        $where = 'plate_id = '.$id;
	       $this->bankuaiRow = $bankuaiObj->getObj($where);
	    }
// 	    print_r($this->bankuaiRow);die;
	    $this->redirect('bankuai_edit');
	}
	
	function check_bankuaiName($name=null,$id=null){
	    //jq的jq.post发过来的参数
	    $bankuai_name = ($name==null) ? IReq::get('bankuai_name','post') : $name;
		$bankuai_id   = ($id==null)   ? IReq::get('bankuai_id','post')   : $id;
		$bankuai_name = IFilter::act($bankuai_name);
		$bankuai_id = intval($bankuai_id);
		
		$bankuaiObj = new IModel('plate');
		if($bankuai_id){
		    //判断名字是否存在
		    $where = 'plate_name ="'.$bankuai_name.'" and plate_id != '.$bankuai_id;
		}else{
		    $where = 'plate_name = "'.$bankuai_name.'"';
		}
		
		$bankuaiRow = $bankuaiObj->getObj($where);
		if(!empty($bankuaiRow)){
		    echo '-1';
		}else{
		    return true;
...
// 	   echo $bankuai_name." ".$bankuai_id;
	...
	//板块提交修改
	function bankuai_edit_act(){
	                                        
	    $plate_id = IFilter::act( IReq::get('id','post') );
	    $plate_name = IFilter::string( IReq::get('plate_name','post') );              //板块名
	    $plate_domainName = IFilter::string( IReq::get('plate_domainName','post') ); //二级域名
	    $plate_describe = IFilter::string( IReq::get('plate_describe','post') );      //描述
	    $plate_isshow = IFilter::string( IReq::get('plate_isshow','post') );       //是否显示
	    //$plate_createTime = time();                                                //创建时间
...
// 	        echo "add";
	        ...
	        $dataArray = array(
	            ...
	            'plate_createTime' =>  $createTime,
	            'plate_updateTime' =>  $createTime,
	            'plate_isshow'     =>  $plate_isshow,
	            'plate_domainName' =>  $plate_domainName,
	            'plate_describe'   => $plate_describe,
	            'plate_creatorIP' => $this->getIp(),
	            'plate_updaterIP' => $this->getIp(),
	        );
	        
	        $bankuaiObj->setData($dataArray);
	        $bankuaiObj->add();
	        ...
	        //获取修改次数
	        $bankuaiRow = $bankuaiObj->getObj($where);
	        $updateCount = $bankuaiRow['plate_updateCount']+1;
            ...
            $dataArray = array(
                'plate_name'       =>  $plate_name,
                'plate_updateTime' =>  $updateTime,
                'plate_isshow'     =>  $plate_isshow,
                'plate_domainName' =>  $plate_domainName,
                'plate_describe'   => $plate_describe,
                'plate_updaterIP' => $this->getIp(),
                'plate_updateCount' =>$updateCount,
            );
            
            
            $bankuaiObj->setData($dataArray);
            $bankuaiObj->update($where);
	    }
	...
	//[板块信息][板块添加][回收站操作]||[物理删除]
	function bankuai_update(){
	    $id = IFilter::act( IReq::get('id') ,'int' );
	    
	    //判断是哪种删除操作方式（回收站删除 or 删除到回收站）
	    $isRecycle = IReq::get('recycle');
	    if(!empty($id)){
	        $obj   = new IModel('plate');
	        $where = Util::joinStr_bankuai($id);
// 	        echo $where ;die;
	        //在回收站里操作的删除就直接删除掉
	        if($isRecycle === null){
	            $obj->del($where);
	            $this->redirect('bankuai_recycle');
	        }else{
	            //把信息删除到回收站
	            //回收站操作类型
	        ...
	        //没有选择的处理
	        if($isRecycle == 'del')
	            $this->redirect('bankuai_list',false);
	        else
	            $this->redirect('bankuai_recycle',false);
	        
	        Util::showMessage('请选择要操作的板块ID');
	    }
	}
	...
	function chatAll(){
	    ...
	    $chat_msg->fields = "COUNT(*) AS num,f_id,f_name,r_state,t_id,t_name";
	    $chat_msg->where  = "r_state = 2 and TIMESTAMPDIFF(HOUR,add_time,'".ITime::getDateTime()."')>1 ";
	    $chat_msg->group  = "f_id,t_id";
	    $date = $chat_msg->find();
	    ...
	    /* if(empty($date)){
    	    ...
    	    $msg = json_encode($date,JSON_UNESCAPED_UNICODE);
	    } */
	    
	    $this->setRenderData(array('msg'=>$date));
	    $this->redirect('chatAll');
	}
	
    
	//读取 A 和 B 的聊天记录
	function getUserchatlog(){
	    ...
	    $users = new IModel('user');
	    $result = $users->query("id in ($f_id,$t_id)");
	...
	function plate_apply(){
	    ...
	    $plate_apply_db = new IQuery('plate_apply as pa');
	    $plate_apply_db->fields = "*";
	    $plate_apply_db->join = "left join plate as p on pa.plate_id=p.plate_id";
	    $plate_apply_db->where = "treatment_result = 1";
	    $res = $plate_apply_db->find();
	    
	   /*  echo "<pre>";
	    print_r($res);
	    echo "</pre>"; */
	    
	    $this->setRenderData(array('apply'=>$res));
	    $this->redirect('plate_apply');
	    //die;
	}
	...
	function apply_deal(){
	    ...
	    $plate_apply_db = new Imodel('plate_apply');
	    $flag = $_GET['id'];//是否同意
	    $seller_id = $_GET['sell_id'];//商家id
	    $deal_no = $_GET['dealid'];//对应申请表id
	    $apply_plate_id = $_GET['plateid'];//申请的板块id
	    
        $dataArray = array(
            "treatment_result"=>$flag
        );
        $plate_apply_db->setData($dataArray);
        $plate_apply_db->update('apply_id = '.$deal_no);
        
        //把申请的板块插入到商家表
        $seller_db = new Imodel('seller');
        $seller = $seller_db->getObj('id = '.$seller_id);
        $seller_plates_json = $seller['plates'];
        $seller_plates = json_decode($seller_plates_json,true);
        array_push($seller_plates,$apply_plate_id);
        //继续打包成json放入到商家表
        $plates_json = json_encode($seller_plates,JSON_UNESCAPED_UNICODE);
        $sellerData = array(
            "plates"=>$plates_json
        );
